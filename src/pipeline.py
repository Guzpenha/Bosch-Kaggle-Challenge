...
if __name__ == "__main__":
	...
	X_train = scipy.sparse.hstack((X_train,X_ate))
	#X_train = scipy.sparse.hstack((X_train,X_date2))
dx = np.where(y_train == 0)[0]
	del X_train_cat
	del X_date
	#del X_date2
	...
	xboost = xgb.XGBClassifier(n_estimators=200, seed=0)
	...
	best_params = val.GridsearchBestRatio(X_train.tocsc(), y_train, pipeline, scoring=score_mcc, verbose=5, ratios=[0.05], params=params)
